...
def run_arcface_stage(student: nn.Module, train_files: List[str], train_dir: Path,
    ...
    for p in student.backbone.parameters():
        p.requires_grad = False
    unfreeze_n = getattr(args, "arcface_unfreeze_blocks", 0)
    if unfreeze_n > 0:
        for block in student.backbone.blocks[-unfreeze_n:]:
            for p in block.parameters():
                p.requires_grad = True
        for p in student.backbone.norm.parameters():
            p.requires_grad = True
        log.info(f"ArcFace stage: unfreezing last {unfreeze_n} backbone blocks")
    ...
    head_params = list(student.projection_head.parameters()) + list(arc_loss_fn.parameters())
    param_groups = [{"params": head_params, "lr": args.arcface_lr}]
    if backbone_params:
        param_groups.append({"params": backbone_params, "lr": args.arcface_lr * 0.1})
...
def parse_args():
    ...
    p.add_argument("--arcface-unfreeze-blocks", type=int, default=0,
                   help="Number of trailing ViT blocks to unfreeze during ArcFace stage")
    p.add_argument("--arcface-only",   action="store_true",
                   help="Skip the DINO stage; load the student from --init-checkpoint "
                        "and run ArcFace fine-tuning only")
    p.add_argument("--init-checkpoint", default=None,
                   help="Student weights to start from when --arcface-only is set "
                        "(a DINO Lightning .ckpt or a raw DinoModel state_dict .pt)")
    return p.parse_args()
...
def main():
    ...
    if args.arcface_only:
        if not args.init_checkpoint:
            sys.exit("[error] --arcface-only requires --init-checkpoint")
        log.info(f"Loading student weights from {args.init_checkpoint}")
        student = DinoModel(args.backbone, args.backbone_dim, args.embed_dim)
        raw = torch.load(args.init_checkpoint, map_location="cpu")
        state_dict = raw.get("state_dict", raw) if isinstance(raw, dict) else raw
        if any(k.startswith("student.") for k in state_dict):
            state_dict = {k[len("student."):]: v for k, v in state_dict.items()
                           if k.startswith("student.")}
        student.load_state_dict(state_dict)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        log.info("Starting ArcFace fine-tuning stage (arcface-only)...")
        run_arcface_stage(student, train_files, train_dir, args, out_dir, device)
        ...
        summary = {
            ...
            "arcface_epochs": args.arcface_epochs,
            "arcface_lr": args.arcface_lr,
            "arcface_unfreeze_blocks": args.arcface_unfreeze_blocks,
